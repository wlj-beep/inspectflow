...
5. Active cadence gate check from latest `Cycle Control Ledger` (no unresolved Red rows)
...
- No unresolved Red controls from test/docs/risk controllers for R1 backlog items.
...
- Control Hub ledger confirms no unresolved Red findings for promoted R2 items.

### R3 Gate
- R1/R2 gates remain green.
- Analytics correctness and multi-site boundary suites pass.
- Control Hub ledger confirms no unresolved Red findings for ANA/INT dependencies.

### R4 Gate
- Full matrix (core + modules + extensions) passes.
- Extension compatibility and rollback paths validated.
- Control Hub ledger confirms no unresolved Red findings across core and extension contracts.
...
- Attach latest `Cycle Control Ledger` and builder next-step closure evidence for release sign-off.

## Stop-the-Line Policy
- Hub gate `Red` freezes new work starts immediately.
- In-flight mitigation may continue only for Red-clearing actions.
- Gate can reopen to `Yellow/Green` only after controller-confirmed mitigation is logged.
- Policy and templates: `docs/operations/persistent-agent-cadence.md`.
