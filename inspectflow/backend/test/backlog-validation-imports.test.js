...
describe("Backlog validation hardening", () => {
  ...
  it("returns row-level warnings when CSV column counts are malformed", async () => {
    const suffix = crypto.randomUUID().slice(0, 5).toUpperCase();
    const jobA = `J-WARN-A-${suffix}`;
    const jobB = `J-WARN-B-${suffix}`;
    const csv = [
      "job_id,part_id,part_revision,op_number,lot,qty,status",
      `${jobA},1234,A,020,Lot Warn,6,open`,
      `${jobB},1234,A,020,Lot Warn,5`
    ].join("\n");
    ...
    const res = await request(app)
      ...
      .send({ csvText: csv });
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ inserted: 2, failed: 0 });
    ...
    const checkA = await query("SELECT id FROM jobs WHERE id=$1", [jobA]);
    const checkB = await query("SELECT id FROM jobs WHERE id=$1", [jobB]);
    expect(checkA.rows[0]).toBeTruthy();
    expect(checkB.rows[0]).toBeTruthy();
  });
