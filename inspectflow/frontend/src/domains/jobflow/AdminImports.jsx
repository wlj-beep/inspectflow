...
export default function AdminImports({
  ...
  async function refreshUnresolved() {
    setLoadingUnresolved(true);
    try {
      const rows = await api.imports.unresolved(currentRole || "Admin", {
        status: "open",
        limit: 100
      });
      setUnresolved(rows || []);
  ...
  async function saveIntegration() {
    ...
    try {
      const normalizedImportType = String(integrationForm.importType || "");
      const options = {};
      if (normalizedImportType === "measurements") {
        if (integrationForm.parserPack) options.parserPack = integrationForm.parserPack;
        if (integrationForm.mappingVersion) options.mappingVersion = integrationForm.mappingVersion;
      }
      await api.imports.createIntegration(
        {
          ...
          pollIntervalMinutes: integrationForm.pollIntervalMinutes
            ? Number(integrationForm.pollIntervalMinutes)
            : null,
      ...
      <div className="card">
        <div className="card-head">
          ...
          <div className="text-muted" style={{ fontSize: ".72rem" }}>
            {loadingTemplates
              ? "Loading templates…"
              : "Use templates to import tools, part dimensions, jobs, and measurement data via CSV."}
          </div>
        </div>
        <div className="card-body">
          ...
          <div className="row2 mt1">
            <div className="card" style={{ margin: 0 }}>
              <div className="card-head">
                <div className="card-title">Tools CSV Import</div>
              </div>
              <div className="card-body">
                <p className="text-muted" style={{ marginTop: 0, fontSize: ".74rem" }}>
                  Upsert tools by name, IT #, type, and visibility fields.
                </p>
                <textarea
                  value={toolsCsv}
                  onChange={(e) => setToolsCsv(e.target.value)}
                  rows={8}
                  placeholder="Paste tools CSV here…"
                  style={{ fontFamily: "var(--mono)", fontSize: ".72rem" }}
                />
                <div className="gap1 mt1">
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => downloadTemplate("tools")}
                  >
                    Download Template
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => setToolsCsv(buildTemplateCsv("tools"))}
                  >
                    Load Sample
                  </button>
                  <button className="btn btn-ghost btn-sm" onClick={() => triggerUpload("tools")}>
                    Upload CSV
                  </button>
                  <button
                    className="btn btn-primary btn-sm"
                    disabled={!canManageTools || running === "tools"}
                    onClick={() => runImport("tools")}
                  >
                ...
                {!canManageTools && (
                  <div className="text-muted" style={{ fontSize: ".72rem", marginTop: ".4rem" }}>
                    Permission required: `manage_tools`.
                  </div>
                )}
              </div>
            </div>
            <div className="card" style={{ margin: 0 }}>
              <div className="card-head">
                <div className="card-title">Part Dimensions CSV Import</div>
              </div>
              <div className="card-body">
                <p className="text-muted" style={{ marginTop: 0, fontSize: ".74rem" }}>
                  Upsert part, operation, and dimension definitions including sampling plan and
                  input mode.
                </p>
                <textarea
                  value={partDimsCsv}
                  onChange={(e) => setPartDimsCsv(e.target.value)}
                  rows={8}
                  placeholder="Paste part dimensions CSV here…"
                  style={{ fontFamily: "var(--mono)", fontSize: ".72rem" }}
                />
                <div className="gap1 mt1">
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => downloadTemplate("partDimensions")}
                  >
                    Download Template
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => setPartDimsCsv(buildTemplateCsv("partDimensions"))}
                  >
                    Load Sample
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => triggerUpload("partDimensions")}
                  >
                    Upload CSV
                  </button>
                  <button
                    className="btn btn-primary btn-sm"
                    disabled={!canManageParts || running === "partDimensions"}
                    onClick={() => runImport("partDimensions")}
                  >
                ...
                {!canManageParts && (
                  <div className="text-muted" style={{ fontSize: ".72rem", marginTop: ".4rem" }}>
                    Permission required: `manage_parts`.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="row2 mt1">
            <div className="card" style={{ margin: 0 }}>
              <div className="card-head">
                <div className="card-title">Jobs CSV Import</div>
              </div>
              <div className="card-body">
                <p className="text-muted" style={{ marginTop: 0, fontSize: ".74rem" }}>
                  Create or update jobs with part/revision/operation validation and row-level error
                  feedback.
                </p>
                <textarea
                  value={jobsCsv}
                  ...
                  rows={8}
                  placeholder="Paste jobs CSV here…"
                  style={{ fontFamily: "var(--mono)", fontSize: ".72rem" }}
                />
                <div className="gap1 mt1">
                  <button className="btn btn-ghost btn-sm" onClick={() => downloadTemplate("jobs")}>
                    Download Template
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => setJobsCsv(buildTemplateCsv("jobs"))}
                  >
                    Load Sample
                  </button>
                  <button className="btn btn-ghost btn-sm" onClick={() => triggerUpload("jobs")}>
                    Upload CSV
                  </button>
                  <button
                    className="btn btn-primary btn-sm"
                    disabled={!canManageJobs || running === "jobs"}
                    onClick={() => runImport("jobs")}
                  >
                ...
                {!canManageJobs && (
                  <div className="text-muted" style={{ fontSize: ".72rem", marginTop: ".4rem" }}>
                    Permission required: `manage_jobs`.
                  </div>
                )}
              </div>
            </div>
            <div className="card" style={{ margin: 0 }}>
              <div className="card-head">
                <div className="card-title">Measurement Bulk Import</div>
              </div>
              <div className="card-body">
                <p className="text-muted" style={{ marginTop: 0, fontSize: ".74rem" }}>
                  Ingest multi-job and multi-operation measurement rows from CMM/API/webhook
                  exports. Ambiguous rows route to Unresolved Imports.
                </p>
                <textarea
                  value={measurementsCsv}
                  onChange={(e) => setMeasurementsCsv(e.target.value)}
                  rows={8}
                  placeholder="Paste measurement CSV here…"
                  style={{ fontFamily: "var(--mono)", fontSize: ".72rem" }}
                />
                <div className="gap1 mt1">
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => downloadTemplate("measurements")}
                  >
                    Download Template
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => setMeasurementsCsv(buildTemplateCsv("measurements"))}
                  >
                    Load Sample
                  </button>
                  <button
                    className="btn btn-ghost btn-sm"
                    onClick={() => triggerUpload("measurements")}
                  >
                    Upload CSV
                  </button>
                  <button
                    className="btn btn-primary btn-sm"
                    disabled={!canManageJobs || running === "measurements"}
                    onClick={() => runImport("measurements")}
                  >
          ...
          <div className="card mt1" style={{ margin: 0 }}>
            <div className="card-head">
              ...
              <div className="text-muted" style={{ fontSize: ".72rem" }}>
                {loadingIntegrations ? "Loading…" : `${integrations.length} configured`}
              </div>
            </div>
            <div className="card-body">
              <div className="row3">
                <div className="field">
                  ...
                  <input
                    ...
                    onChange={(e) => setIntegrationForm((p) => ({ ...p, name: e.target.value }))}
                    placeholder="e.g. Customer A Jobs Feed"
                  />
                </div>
                <div className="field">
                  <label>Source</label>
                  <select
                    value={integrationForm.sourceType}
                    onChange={(e) =>
                      setIntegrationForm((p) => ({ ...p, sourceType: e.target.value }))
                    }
                  >
                ...
                <div className="field">
                  ...
                  <select
                    value={integrationForm.importType}
                    onChange={(e) =>
                      setIntegrationForm((p) => ({ ...p, importType: e.target.value }))
                    }
                  >
              ...
              <div className="row3 mt1">
                <div className="field">
                  ...
                  <input
                    value={integrationForm.endpointUrl}
                    onChange={(e) =>
                      setIntegrationForm((p) => ({ ...p, endpointUrl: e.target.value }))
                    }
                ...
                <div className="field">
                  ...
                  <input
                    type="number"
                    min="1"
                    value={integrationForm.pollIntervalMinutes}
                    onChange={(e) =>
                      setIntegrationForm((p) => ({ ...p, pollIntervalMinutes: e.target.value }))
                    }
                    placeholder="15"
                  />
                </div>
                <div className="field" style={{ display: "flex", alignItems: "flex-end" }}>
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={integrationForm.enabled !== false}
                      onChange={(e) =>
                        setIntegrationForm((p) => ({ ...p, enabled: e.target.checked }))
                      }
                    />
                    Enabled
                  </label>
              ...
              {integrationForm.importType === "measurements" && (
                <div className="row3 mt1">
                  <div className="field">
                    <label>Parser Pack</label>
                    <select
                      value={integrationForm.parserPack}
                      onChange={(e) =>
                        setIntegrationForm((p) => ({ ...p, parserPack: e.target.value }))
                      }
                    >
                      <option value="">Auto detect</option>
                      {(templates?.metrologyParserPacks?.packs || []).map((pack) => (
                  ...
                  <div className="field">
                    ...
                    <input
                      value={integrationForm.mappingVersion}
                      onChange={(e) =>
                        setIntegrationForm((p) => ({
                          ...p,
                          mappingVersion: e.target.value || "v1"
                        }))
                      }
                      placeholder="v1"
                    />
                  ...
                  <div className="field" style={{ justifyContent: "flex-end" }}>
                    <div className="text-muted" style={{ fontSize: ".72rem" }}>
                      Controls parser contract versioning for bubbling integration runs.
                    </div>
                  </div>
                </div>
              )}
              <div className="mt1">
                <button
                  className="btn btn-primary btn-sm"
                  disabled={!canManageJobs || integrationBusy === "save"}
              ...
              <table className="data-table mt1">
                ...
                <tbody>
                  {integrations.length === 0 && (
                    <tr>
                      <td colSpan={10}>
                        <div className="empty-state">No integrations configured.</div>
                      </td>
                    </tr>
                  )}
                  {integrations.map((integ) => (
                    <tr key={integ.id}>
                      ...
                      <td className="mono" style={{ fontSize: ".7rem" }}>
                        {integ.options?.parserPack || "—"}
                      </td>
                      <td className="mono" style={{ fontSize: ".7rem" }}>
                        {integ.options?.mappingVersion || "—"}
                      </td>
                      <td
                        className="mono"
                        style={{ fontSize: ".7rem", maxWidth: "340px", whiteSpace: "normal" }}
                      >
                        {integ.endpoint_url || "—"}
                      </td>
                      <td className="mono">{integ.poll_interval_minutes || "—"}</td>
                      <td className="mono" style={{ fontSize: ".7rem" }}>
                        {integ.last_run_at ? fmtTs(integ.last_run_at) : "—"}
                      </td>
                      <td>
                        {integ.last_status ? (
                          <span
                            className={`badge ${integ.last_status === "success" ? "badge-ok" : integ.last_status === "partial" ? "badge-pend" : "badge-incomplete"}`}
                          >
                            {integ.last_status}
                          </span>
                        ) : (
                      ...
                      <td>
                        <button
                          className="btn btn-ghost btn-sm"
                          disabled={!canManageJobs || integrationBusy === String(integ.id)}
                          onClick={() => pullIntegration(integ.id)}
                        >
          ...
          <div className="card mt1" style={{ margin: 0 }}>
            <div className="card-head">
              ...
              <div className="text-muted" style={{ fontSize: ".72rem" }}>
                {loadingUnresolved ? "Loading…" : `${unresolved.length} open`}
              </div>
            </div>
            <div className="card-body">
              <p className="text-muted" style={{ marginTop: 0, fontSize: ".74rem" }}>
                Rows with low confidence or missing mappings appear here. Override fields and
                resolve manually, or ignore if not needed.
              </p>
              <div
                className="mt1"
              ...
              >
                {unresolved.length === 0 && (
                  <div className="empty-state">No unresolved import rows.</div>
                )}
                {unresolved.map((item) => {
                  const edit = editForUnresolved(item);
                    ...
                    >
                      ...
                      >
                        <div style={{ fontSize: ".76rem" }}>
                          ...
                          <span className="text-warn">{item.reason}</span> · line{" "}
                          {item.line_number || "?"}
                        </div>
                        <div className="mono" style={{ fontSize: ".68rem", opacity: 0.7 }}>
                          conf {item.confidence || "n/a"}
                        </div>
                      </div>
                      <div className="row3 mt1">
                        <input
                          value={edit.jobId}
                          onChange={(e) => setUnresolvedEdit(item.id, { jobId: e.target.value })}
                          placeholder="job id"
                        />
                        <input
                          value={edit.operationId}
                          onChange={(e) =>
                            setUnresolvedEdit(item.id, { operationId: e.target.value })
                          }
                          placeholder="operation id"
                        />
                        <input
                          value={edit.partId}
                          onChange={(e) => setUnresolvedEdit(item.id, { partId: e.target.value })}
                          placeholder="part id"
                        />
                      </div>
                      <div className="row3 mt1">
                        <input
                          value={edit.dimensionId}
                          onChange={(e) =>
                            setUnresolvedEdit(item.id, { dimensionId: e.target.value })
                          }
                          placeholder="dimension id"
                        />
                        <input
                          value={edit.dimensionName}
                          onChange={(e) =>
                            setUnresolvedEdit(item.id, { dimensionName: e.target.value })
                          }
                          placeholder="dimension name"
                        />
                        <input
                          value={edit.pieceNumber}
                          onChange={(e) =>
                            setUnresolvedEdit(item.id, { pieceNumber: e.target.value })
                          }
                          placeholder="piece #"
                        />
                      </div>
                      <div className="row3 mt1">
                        <input
                          value={edit.value}
                          onChange={(e) => setUnresolvedEdit(item.id, { value: e.target.value })}
                          placeholder="value"
                        />
                        <input
                          value={edit.operatorUserId}
                          onChange={(e) =>
                            setUnresolvedEdit(item.id, { operatorUserId: e.target.value })
                          }
                          placeholder="operator user id"
                        />
                        <input
                          value={edit.status}
                          onChange={(e) => setUnresolvedEdit(item.id, { status: e.target.value })}
                      ...
                      <div className="mt1" style={{ display: "flex", gap: ".45rem" }}>
                        ...
                        >
                          {busyResolve ? "Resolving…" : "Resolve"}
                        </button>
                        <button
                          className="btn btn-ghost btn-sm"
                          disabled={busyIgnore}
                          onClick={() => ignoreUnresolved(item)}
                        >
                          {busyIgnore ? "Ignoring…" : "Ignore"}
                        </button>
                      </div>
                    </div>
                  );
