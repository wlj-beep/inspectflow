...
import { parseFeatureModifiersInput } from "./jobflowUtils.js";
...
import { InstructionVersionManager, ToolSearchPopover } from "./jobflowWidgets.jsx";
...
export default function AdminParts({
  ...
  const [dimensionAuditById, setDimensionAuditById] = useState({});
  const [dimensionAuditLoadingId, setDimensionAuditLoadingId] = useState("");
  const [dimensionAuditError, setDimensionAuditError] = useState("");
  useEffect(() => {
  ...
  const visiblePartEntries = filteredPartEntries.slice(
    (safePage - 1) * pageSize,
    safePage * pageSize
  );
  useEffect(() => {
  ...
  async function handleAddPart() {
    ...
    const revision = String(newPart.revision || "")
      .trim()
      .toUpperCase();
    if (!pn || !newPart.description.trim() || !/^[A-Z]+$/.test(revision)) {
      setPartErr("Part number, part name, and revision are required.");
      return;
    }
    if (parts[pn]) {
      setPartErr("Part number already exists.");
      return;
    }
    setPartErr("");
    setApiErr("");
  ...
  function updateDescLocal(pn, v) {
    onUpdatePart(pn, v, false).catch((err) => { console.warn("[part-update] updateDescLocal:", err?.message || err); });
  }
  function updateDescPersist(pn, v) {
    ...
    if (!confirmRevisionCommit(pn, "Update part name")) {
      onUpdatePart(pn, persistedName, false).catch((err) => { console.warn("[part-update] rollback:", err?.message || err); });
      return;
    }
  ...
  async function handleAddOp(pn) {
    ...
    if (!opKey) {
      setApiErr("Operation number must be between 001 and 999.");
      return;
    }
    if (parts[pn].operations[opKey]) {
      setApiErr(`Operation ${opKey} already exists.`);
      return;
    }
  ...
  function updateDim(pn, opKey, dimId, field, value, persist, dimName) {
    ...
    onUpdateDim(pn, opKey, dimId, field, value, persist).catch((e) =>
      setApiErr(e?.message || "Unable to update dimension.")
    );
  }
  ...
  async function removeDim(pn, opKey, dimId, dimName) {
    if (!confirmRevisionCommit(pn, `Remove dimension ${dimName || dimId} from operation ${opKey}`))
      return;
    setApiErr("");
    try {
  ...
  async function loadDimensionAudit(dimId) {
    if (!dimId || dimensionAuditLoadingId) return;
    setDimensionAuditError("");
    setDimensionAuditLoadingId(String(dimId));
    try {
      const response = await api.dimensions.characteristicAudit(dimId, currentRole || "Admin", 25);
      setDimensionAuditById((prev) => ({
        ...prev,
        [String(dimId)]: Array.isArray(response?.entries) ? response.entries : []
      }));
    } catch (e) {
      setDimensionAuditError(e?.message || "Unable to load characteristic audit.");
    } finally {
      setDimensionAuditLoadingId("");
    }
  }
  async function handleBulkRename() {
    const find = bulkFind;
    if (!find.trim()) {
      setBulkMsg("Enter text to find.");
      return;
    }
    ...
    const updates = filteredPartEntries
      ...
      .filter(Boolean);
    if (updates.length === 0) {
      setBulkMsg("No filtered parts matched the find text.");
      return;
    }
    ...
    try {
      ...
      setBulkMsg(
        `Bulk update complete: ${updated} updated, ${skipped} skipped${missing ? `, ${missing} missing` : ""}.`
      );
    } catch (e) {
    ...
    <div>
      <div className="card">
        ...
        <div className="card-body">
          <div className="row3">
            <div className="field">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewPart((p) => ({ ...p, partNumber: e.target.value.toUpperCase() }))
                }
            ...
            <div className="field">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewPart((p) => ({
                    ...p,
                    revision: e.target.value.toUpperCase().replace(/[^A-Z]/g, "")
                  }))
                }
                placeholder="A"
                style={{ fontFamily: "var(--mono)" }}
          ...
          <div className="mt2">
            <button className="btn btn-primary" disabled={saving} onClick={handleAddPart}>
      ...
      <div className="card">
        <div className="card-head">
          <div className="card-title">Catalog Controls</div>
        </div>
        <div className="card-body">
          <div className="row3">
            <div className="field">
              ...
              <input
                ...
                placeholder="Filter by part number, name, or revision"
              />
            </div>
            <div className="field">
            ...
            <div className="field" style={{ alignItems: "flex-end" }}>
              <div className="text-muted" style={{ fontSize: ".72rem" }}>
                Showing {visiblePartEntries.length} of {filteredPartEntries.length} filtered (
                {sortedPartEntries.length} total)
              </div>
            </div>
          </div>
          <div className="row3 mt1">
            <div className="field">
              ...
              <input
                ...
                placeholder="Find text in part names"
              />
            </div>
            <div className="field">
              <label>Bulk Replace</label>
              <input
                value={bulkReplace}
            ...
            <div className="field" style={{ justifyContent: "flex-end" }}>
              <button className="btn btn-ghost" disabled={bulkBusy} onClick={handleBulkRename}>
                {bulkBusy ? "Applying…" : "Apply to Filtered Parts"}
              </button>
            </div>
          </div>
          {bulkMsg && (
            <p className="text-muted mt1" style={{ fontSize: ".74rem" }}>
              {bulkMsg}
            </p>
          )}
          <div className="gap1 mt1" style={{ justifyContent: "flex-end" }}>
            ...
            <div className="text-muted" style={{ fontSize: ".72rem", padding: ".35rem .4rem" }}>
          ...
          <div className="card-head">
            <div style={{ display: "flex", alignItems: "center", gap: "1rem", flex: 1 }}>
              ...
              <input
                ...
                onChange={(e) => updateDescLocal(pn, e.target.value)}
                style={{
                ...
                onBlur={(e) => {
                  updateDescPersist(pn, e.target.value);
                  e.target.style.borderColor = "var(--border2)";
                }}
              />
              <div className="text-muted" style={{ fontSize: ".72rem", whiteSpace: "nowrap" }}>
                Rev {part.currentRevision || "A"} →{" "}
                {part.nextRevision || nextRevisionCode(part.currentRevision || "A")}
              </div>
            </div>
          </div>
          <div style={{ padding: "1rem 1.25rem" }}>
                ...
                <div style={{ marginTop: ".4rem", display: "grid", gap: ".2rem" }}>
                  {part.revisions.slice(0, 10).map((rev) => (
                    <div key={rev.revision} className="text-muted" style={{ fontSize: ".72rem" }}>
                      Rev {rev.revision} · {rev.changeSummary || "Setup update"} ·{" "}
                      {fmtTs(rev.createdAt)}
                    </div>
                  ))}
                </div>
            ...
            {Object.entries(part.operations).map(([opKey, op]) => (
              <div key={opKey} style={{ marginBottom: "1.5rem" }}>
                      ...
                      {op.dimensions.length === 0 && (
                        <tr>
                          <td
                            ...
                            style={{ padding: "1rem", fontSize: ".76rem" }}
                          >
                        ...
                        <tr key={d.id}>
                          <td>
                            <input
                              value={d.name}
                              onChange={(e) =>
                                updateDim(pn, opKey, d.id, "name", e.target.value, false, d.name)
                              }
                              onBlur={(e) =>
                                updateDim(pn, opKey, d.id, "name", e.target.value, true, d.name)
                              }
                            />
                            <div className="row3" style={{ marginTop: ".35rem" }}>
                              <input
                                value={d.bubbleNumber || ""}
                                onChange={(e) =>
                                  updateDim(
                                    pn,
                                    opKey,
                              ...
                              <input
                                ...
                                placeholder="GD&T class"
                              />
                            </div>
                            <div className="row3" style={{ marginTop: ".35rem" }}>
                              <input
                                value={d.toleranceZone || ""}
                                onChange={(e) =>
                                  updateDim(
                                    pn,
                                    opKey,
                                ...
                                placeholder="Tolerance zone"
                              />
                              <input
                                value={d.featureQuantity ?? ""}
                                onChange={(e) =>
                                  updateDim(
                                    pn,
                                    opKey,
                            ...
                            <div className="row3" style={{ marginTop: ".35rem" }}>
                              <input
                                value={
                                  Array.isArray(d.featureModifiers)
                                    ? d.featureModifiers.join(";")
                                    : ""
                                }
                                onChange={(e) =>
                                  updateDim(
                                    ...
                                    d.id,
                                    "featureModifiers",
                                    parseFeatureModifiersInput(e.target.value),
                                    false,
                                    d.name
                                  )
                                }
                                ...
                                placeholder="Modifiers (MMC;DATUM_A)"
                              />
                              <input
                                value={d.sourceCharacteristicKey || ""}
                                onChange={(e) =>
                                  updateDim(
                                    pn,
                                    opKey,
                                ...
                                placeholder="Source characteristic key"
                              />
                              <button
                                className="btn btn-ghost btn-sm"
                                disabled={dimensionAuditLoadingId === String(d.id)}
                                onClick={() => loadDimensionAudit(d.id)}
                              >
                                {dimensionAuditLoadingId === String(d.id) ? "Loading…" : "Audit"}
                              </button>
                            </div>
                            {dimensionAuditError && (
                              <div className="err-text" style={{ marginTop: ".35rem" }}>
                                {dimensionAuditError}
                              </div>
                            )}
                            {Array.isArray(dimensionAuditById[String(d.id)]) &&
                              dimensionAuditById[String(d.id)].length > 0 && (
                                <details style={{ marginTop: ".35rem" }}>
                                  <summary style={{ cursor: "pointer" }}>
                                    Schema Audit ({dimensionAuditById[String(d.id)].length})
                                  </summary>
                                  <div
                                    style={{ display: "grid", gap: ".25rem", marginTop: ".25rem" }}
                                  >
                                    {dimensionAuditById[String(d.id)].map((entry) => (
                                      <div
                                        key={entry.id}
                                        className="text-muted"
                                        style={{ fontSize: ".7rem" }}
                                      >
                                        {fmtTs(entry.created_at)} · {entry.action} ·{" "}
                                        {entry.actor_role || "system"} ·{" "}
                                        {entry.source || "admin_ui"}
                                      </div>
                                    ))}
                                  </div>
                                </details>
                          ...
                          <td>
                            <select
                              value={d.unit}
                              onChange={(e) =>
                                updateDim(pn, opKey, d.id, "unit", e.target.value, true, d.name)
                              }
                            >
                              <option>in</option>
                              <option>mm</option>
                          ...
                          <td>
                            <select
                              value={d.sampling}
                              onChange={(e) =>
                                updateDim(pn, opKey, d.id, "sampling", e.target.value, true, d.name)
                              }
                            >
                          ...
                          <td>
                            <ToolSearchPopover
                              ...
                              selectedIds={d.tools}
                              onAdd={(id) =>
                                updateDim(pn, opKey, d.id, "tools", [...d.tools, id], true, d.name)
                              }
                              onRemove={(id) =>
                                updateDim(
                                  pn,
                          ...
                          <td style={{ textAlign: "center" }}>
                            <button
                              ...
                              onClick={() => removeDim(pn, opKey, d.id, d.name)}
                            >
                              ✕
                            </button>
                          </td>
                ...
                <button className="btn btn-ghost btn-sm" onClick={() => addDim(pn, opKey)}>
                  + Add Dimension
                </button>
            ...
            >
              ...
              <div className="row3" style={{ gap: ".75rem" }}>
                <div className="field">
                  ...
                  <input
                    value={newOp[pn]?.opNum || ""}
                    onChange={(e) =>
                      setNewOp((p) => ({ ...p, [pn]: { ...p[pn], opNum: e.target.value } }))
                    }
                    placeholder="e.g. 040"
                    style={{ fontFamily: "var(--mono)" }}
                  />
                ...
                <div className="field">
                  ...
                  <input
                    value={newOp[pn]?.label || ""}
                    onChange={(e) =>
                      setNewOp((p) => ({ ...p, [pn]: { ...p[pn], label: e.target.value } }))
                    }
                    placeholder="e.g. Final Inspection"
                  />
                </div>
                <div className="field" style={{ justifyContent: "flex-end" }}>
                  <button className="btn btn-ghost" onClick={() => handleAddOp(pn)}>
                    + Add Operation
                  </button>
                </div>
              </div>
